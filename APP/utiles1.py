...
class objectdetection_countingregion:
    ...
    def get_chart(self):
        with self.lock:
            ...
            ax1.bar(self.date, self.linecount1, color='b')
            ax1.set_title('Car Count In Zone', fontsize=12)
            ax1.tick_params(axis='x', rotation=45, labelsize=10)
            ax1.tick_params(axis='y', labelsize=10)
            ax1.yaxis.set_major_locator(MaxNLocator(integer=True))
            ax1.grid(True)
            ax2.bar(self.date, self.linecount2, color='r')
            ax2.set_title('Car Count Out Zone', fontsize=12)
            ...
            ax3.set_title('Car Count Polygon Zone 1', fontsize=12)
            ax3.tick_params(axis='x', rotation=45, labelsize=10)
            ax3.tick_params(axis='y', labelsize=10)
            ax3.yaxis.set_major_locator(MaxNLocator(integer=True))
            ax3.grid(True)
            ax4.bar(self.datetime, self.polygon2count, color='r')
            ax4.set_title('Car Count Polygon Zone 2', fontsize=12)
